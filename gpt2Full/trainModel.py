...
from com.env import MODEL_ORIGIN_PATH, DATA_PREPROCESS_PATH, MODEL_FINETUNE_PATH
...
def main():
    ...
    tokenizer_path = os.path.join(MODEL_ORIGIN_PATH, model_name)
    ...
    # 데이터셋 로드
    dataset = load_from_disk(data_preprocess_path)
    ...
    training_args = TrainingArguments(
        ...
        num_train_epochs=3,
        logging_dir=os.path.join(model_finetune_path, "logs"),
        save_steps=100,
        save_total_limit=2,
    ...
    # 모델 저장
    model.save_pretrained(model_finetune_path)
    tokenizer.save_pretrained(model_finetune_path)
    print(f"모델과 토크나이저가 {model_finetune_path}에 저장되었습니다.")
