...
def get_advice(query):
    ...
    print(model_finetune_path)

    try:
        # 토크나이저 로드
        tokenizer = AutoTokenizer.from_pretrained(model_finetune_path)
        ...
        try:
            # LoRA 모델인 경우
            config = PeftConfig.from_pretrained(model_finetune_path)
            print(f"LoRA 모델 감지됨. 기본 모델: {model_origin_path}")
            ...
            # LoRA 어댑터 적용
            model = PeftModel.from_pretrained(base_model, model_finetune_path)
